...
public:
   ADNonlinearFormIntegrator(ADFunction2 &f, IntegrationRule *ir = nullptr)
      : NonlinearFormIntegrator(ir), f(f), vdim(1) {}
   ADNonlinearFormIntegrator(ADFunction2 &f, int vdim,
...
private:
   ...
   // DenseMatrix d2shape, d2shape1, d2shape2; // for hessian. Not implemented yet.
public:
   ADBlockNonlinearFormIntegrator(ADFunction2 &f,
   ...
   ADBlockNonlinearFormIntegrator(ADFunction2 &f, std::initializer_list<int> vdim,
                                  const IntegrationRule *ir = nullptr)
      : ADBlockNonlinearFormIntegrator(f, ir), vdim(vdim)
   {}

   ADBlockNonlinearFormIntegrator(ADFunction2 &f, const Array<int> &vdim,
                                  const IntegrationRule *ir = nullptr)
