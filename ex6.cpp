...
MAKE_AD_FUNCTION(VolumeConstraint, T, V, M, x, dummy,
{
   // x[0] is the value of the constraint
   // x[1] is the gradient of the constraint
   return x[0];
});
MAKE_AD_FUNCTION(MassConstraint, T, V, M, x, dummy,
{
   // x[0] is the value of the constraint
   // x[1] is the gradient of the constraint
   return x[0]*x[1];
});
MAKE_AD_FUNCTION(TotalEnergyConstraint, T, V, M, x, dummy,
{
   // x[0] is the value of the constraint
   // x[1] is the gradient of the constraint
   T result = T();
...
template <int comp>
MAKE_AD_FUNCTION(MomentumConstraint, T, V, M, x, dummy,
{
   // x[0] is the value of the constraint
   // x[1] is the gradient of the constraint
   // x[2] is the momentum component
   return x[0]*x[1]*x[3 + comp];
});
...
MakeConstraints(const Vector &target, Vector &lambda, real_t &mu, const int dim)
{
   ...
   const_list.emplace_back(std::make_unique<VolumeConstraint>(numVars));
   const_list.emplace_back(std::make_unique<MassConstraint>(numVars));
   const_list.emplace_back(std::make_unique<TotalEnergyConstraint>(numVars));
   if (dim >= 1)
   {
      const_list.emplace_back(std::make_unique<MomentumConstraint<0>>(numVars));
   }
   if (dim >= 2)
   {
      const_list.emplace_back(std::make_unique<MomentumConstraint<1>>(numVars));
   }
   if (dim == 3)
   {
      const_list.emplace_back(std::make_unique<MomentumConstraint<1>>(numVars));
   ...
   for (int i=0; i<numVars; i++)
   {
      entropy_list[i] = std::make_unique<FermiDiracEntropy>(1, 2);
      if (i == 0)
      {
         PGAL_energy_list[i] = std::make_unique<ADPGEnergy>(ADPGEnergy(
                                  *TotalAL[numVars-1], *entropy_list[i]));
      }
      else
      {
...
int main(int argc, char *argv[])
{
   ...
   Vector x(numVars*2);
   Vector param(numVars*4);
   x = 0.0;
   param = 1.0;
   Vector dummy;

   auto obj_and_temp = MakeConstraints(const_targets, lambda, mu, dim);
